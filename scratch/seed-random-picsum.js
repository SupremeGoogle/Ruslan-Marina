...
async function seedData() {
  ...
  for (let idx = 0; idx < createdGuests.length; idx++) {
    ...
    console.log(`Seeding ${photoCount} random photos for ${name}...`);

    for (let p = 0; p < photoCount; p++) {
      // Use standard random picsum url query parameters to guarantee they load and never 404
      const width = p % 2 === 0 ? 800 : 600;
      const height = p % 2 === 0 ? 600 : 800;
      const url = `https://picsum.photos/${width}/${height}?random=${idx}_${p}`;
  ...
  console.log('\nDatabase seeding completed successfully with 18 guests and random photos!');
}
