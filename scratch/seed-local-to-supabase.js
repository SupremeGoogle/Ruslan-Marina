...
async function seedData() {
  ...
  // Load local images from public folder
  const couplePath = path.join(__dirname, '..', 'public', 'wedding_couple.png');
  const ringsPath = path.join(__dirname, '..', 'public', 'wedding_rings.png');

  if (!fs.existsSync(couplePath) || !fs.existsSync(ringsPath)) {
    console.error('Error: public/wedding_couple.png or public/wedding_rings.png missing in project.');
    process.exit(1);
  }

  const coupleBuffer = fs.readFileSync(couplePath);
  const ringsBuffer = fs.readFileSync(ringsPath);
  
  const sampleImages = [
    { buffer: coupleBuffer, contentType: 'image/png', ext: 'png' },
    { buffer: ringsBuffer, contentType: 'image/png', ext: 'png' }
  ];
  ...
  for (let idx = 0; idx < createdGuests.length; idx++) {
    ...
    for (let p = 0; p < photoCount; p++) {
      // Pick local wedding image alternatively
      const imageIndex = p % sampleImages.length;
      const { buffer, contentType, ext } = sampleImages[imageIndex];
      
      const fileName = `seed/${guest.id}/${Date.now()}_${p}_${Math.random().toString(36).substring(2, 7)}.${ext}`;
